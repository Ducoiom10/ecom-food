...
<div class="h-full overflow-y-auto bg-[#0F0F0F] p-4 lg:p-6">
  ...
  <div class="grid grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4 mb-6">
    @foreach([
      ['label'=>'Doanh thu hôm nay','value'=>number_format($revenue??0).'đ',        'change'=>'+12%',        'icon'=>'💰','color'=>'text-green-400'],
      ['label'=>'Tổng đơn',         'value'=>($orderCount??0).' đơn',                'change'=>'+8%',         'icon'=>'🛍','color'=>'text-blue-400'],
      ['label'=>'Đánh giá TB',      'value'=>($avgRating??0).' ⭐',                   'change'=>'+0.1',        'icon'=>'⭐','color'=>'text-yellow-400'],
      ['label'=>'TB đơn/giờ',       'value'=>round(($orderCount??0)/max(1,14)).' đơn','change'=>'bình thường', 'icon'=>'📈','color'=>'text-orange-400'],
    ] as $kpi)
  ...
  <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 lg:gap-6">
    ...
    <div class="xl:col-span-2 bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4">
      ...
      <div class="h-40 lg:h-52 flex items-end gap-1 px-2">
        ...
        @empty
        <div class="w-full flex items-center justify-center text-gray-600 text-xs">Chưa có dữ liệu hôm nay</div>
        @endforelse
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="bg-[#1A1A1A] border-2 border-red-500/30 rounded-2xl p-4">
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full ml-auto">{{ $lowStock->count() }}</span>
        </div>
        <div class="space-y-2">
          @forelse($lowStock as $item)
          <div class="flex items-center justify-between text-xs rounded-xl px-3 py-2
            ...
            <div class="text-right">
              <span class="font-black {{ $item->current_qty <= 0 ? 'text-red-400' : 'text-yellow-400' }}">
                {{ $item->current_qty }}/{{ $item->min_threshold }} {{ $item->unit }}
              </span>
              <div class="text-[9px] {{ $item->current_qty <= 0 ? 'text-red-400' : 'text-yellow-400' }}">
      ...
      <div class="bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4">
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="bg-[#FF6B35] text-white text-[10px] font-black px-1.5 py-0.5 rounded-full ml-auto">{{ $refunds->count() }}</span>
        </div>
        <div class="space-y-3">
          @forelse($refunds as $refund)
          <div class="rounded-xl p-3 border bg-orange-900/20 border-orange-700/30">
            <div class="flex items-center justify-between mb-1">
              <span class="text-white text-xs font-black">{{ $refund->order_number }}</span>
            ...
            <span class="text-[#FF6B35] font-black text-sm">{{ number_format($refund->grand_total) }}đ</span>
          </div>
          @empty
