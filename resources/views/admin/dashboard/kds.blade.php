                    ...
                    <div
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-white font-black text-sm uppercase tracking-wide">{{ $col['title'] }}</span>
                            </div>
                            <span
                                class="{{ $col['color'] }} text-white text-xs font-black w-6 h-6 rounded-full flex items-center justify-center">{{ $colOrders->count() }}</span>
                        </div>
            ...
            {{ $order->priority === 'high' ? 'border-[#FF6B35] shadow-[4px_4px_0px_#FF6B35]' : 'border-[#333] shadow-[4px_4px_0px_#333]' }}">
                                    ...
                                    <div class="px-4 py-3 border-b border-[#333] flex items-center justify-between">
                                        <div>
                                            ...
                                            <div class="text-gray-400 text-xs">
                                                {{ $order->delivery_mode === 'delivery' ? '🛵 Giao hàng' : '🏪 Tự lấy' }}
                                            </div>
                                        </div>
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    @if ($item->options->isNotEmpty())
                                                        <div class="text-[#FFD23F] text-xs mt-0.5">+
                                                            {{ $item->options->map(fn($o) => $o->optionValue->label)->implode(', ') }}
                                                        </div>
                                                    @endif
                                                    @if ($item->note)
                                    ...
                                    <div class="px-4 pb-4">
                                        <form action="{{ route($col['route'], $order->id) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="w-full py-3 rounded-xl font-black text-sm border-2 border-[#444] transition-all {{ $col['actionColor'] }}">
                                                {{ $col['action'] }}
                                            </button>
                                        </form>
                                    </div>
                            ...
                            <div class="flex items-center gap-4">
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs text-gray-400 mb-2">Còn: <span
                                            class="text-white font-bold">{{ $item->current_qty }}
                                            {{ $item->unit }}</span> / Ngưỡng: {{ $item->min_threshold }}</div>
                                    <div class="h-2 bg-[#333] rounded-full overflow-hidden">
                                        <div class="h-full rounded-full {{ $item->current_qty <= 0 ? 'bg-red-500' : ($item->isLow() ? 'bg-yellow-500' : 'bg-green-500') }}"
                                            style="width: {{ $item->max_qty > 0 ? min(100, ($item->current_qty / $item->max_qty) * 100) : 0 }}%">
                                        </div>
                                    </div>
                                </div>
                                <form action="{{ route('admin.kds.inventory', $item->id) }}" method="POST">
                                    ...
                                    <input type="hidden" name="current_qty"
                                        value="{{ $item->current_qty <= 0 ? $item->max_qty : 0 }}" />
                                    <button type="submit"
                                        class="w-16 h-8 rounded-full border-2 transition-all flex items-center {{ $item->current_qty > 0 ? 'bg-green-500 border-green-500 justify-end' : 'bg-gray-700 border-gray-600 justify-start' }}">
