      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        ...
        <div class="lg:col-span-2 bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4">
          ...
          <div class="h-40 lg:h-52 flex items-end gap-2 px-2">
            ...
            @empty
            <div class="w-full flex items-center justify-center text-gray-600 text-xs">Chưa có dữ liệu</div>
            @endforelse
          </div>
        </div>
          ...
          <div class="space-y-3">
            ...
            <div>
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-white font-black text-xs">{{ number_format($branchRevenue/1000000,1) }}M</span>
              </div>
              <div class="h-3 bg-[#333] rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-[#FF6B35] to-[#FFD23F] rounded-full" style="width: {{ min(100,($branchRevenue/max(1,16000000))*100) }}%"></div>
              </div>
            </div>
            @endforeach
          </div>
          ...
          <table class="w-full min-w-[600px]">
            ...
            <tbody>
              ...
              <tr class="border-b border-[#222] hover:bg-[#1D1D1D]">
                ...
                <td class="px-4 py-3">
                  <div class="text-white text-sm font-bold">{{ $v->used_count }}/{{ $v->max_uses ?? '∞' }}</div>
                  @if($v->max_uses)
                  <div class="h-1.5 bg-[#333] rounded-full mt-1 w-20">
                    <div class="h-full bg-[#FF6B35] rounded-full" style="width: {{ ($v->used_count/max(1,$v->max_uses))*100 }}%"></div>
                  </div>
                  @endif
                </td>
                <td class="px-4 py-3 text-gray-400 text-sm">{{ $v->expires_at ? $v->expires_at->format('d/m/Y') : '∞' }}</td>
                <td class="px-4 py-3">
                  <span class="text-xs font-black px-2 py-1 rounded-full {{ $v->isValid() ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-700 text-gray-500' }}">
                    {{ $v->isValid() ? 'Hoạt động' : 'Hết hạn' }}
                  </span>
                </td>
              </tr>
              @empty
              <tr><td colspan="6" class="text-center text-gray-500 py-6 text-sm">Chưa có voucher nào</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
        ...
        <form action="{{ route('admin.super.push') }}" method="POST" class="space-y-3">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input name="title" placeholder="🔥 Flash Sale giờ trưa!" class="w-full bg-[#222] border border-[#444] text-white rounded-xl px-3 py-2 text-sm outline-none focus:border-[#FF6B35]" />
            </div>
            <div>
          ...
          <textarea name="body" placeholder="Nội dung thông báo..." class="w-full bg-[#222] border border-[#444] text-white rounded-xl px-3 py-2 text-sm outline-none focus:border-[#FF6B35] h-20 resize-none"></textarea>
          ...
          <table class="w-full min-w-[700px]">
            ...
            <tbody>
              ...
              <tr class="border-b border-[#1D1D1D] hover:bg-[#1D1D1D] font-mono text-xs">
                <td class="px-4 py-3 text-gray-500">{{ $log->created_at->format('H:i:s') }}</td>
              ...
              @empty
              <tr><td colspan="6" class="text-center text-gray-500 py-6 text-sm">Chưa có audit log nào</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
