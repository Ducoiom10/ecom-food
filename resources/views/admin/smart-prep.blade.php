...
<div class="h-full overflow-y-auto bg-[#0F0F0F] p-6">
  ...
  <div class="bg-red-600 border-2 border-red-400 rounded-2xl p-4 mb-6 flex items-center gap-3">
    ...
    <div class="flex-1">
      <p class="text-white font-black">🚨 {{ $criticalCount }} cảnh báo KHẨN CẤP, {{ $highCount }} cảnh báo CAO!</p>
    ...
    <div class="space-y-4">
      <div class="bg-gradient-to-br from-[#1A2040] to-[#0D1520] border-2 border-[#2A3A6A] rounded-2xl p-5">
        ...
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            <div class="text-white font-black text-3xl">{{ $weather['temp'] }}°C</div>
            <div class="text-gray-400 text-sm">{{ $weather['label'] }}</div>
          </div>
        </div>
        <div class="rounded-xl p-3 text-xs font-bold bg-blue-900/50 text-blue-300 border border-blue-700/50">
          {{ $weather['impact'] }}
        </div>
        @if($weather['deliveryBoost'] > 0)
        ...
        <div class="mt-3">
          ...
          <div class="flex gap-1">
            ...
            <a href="{{ route('admin.smartprep', ['weather' => $w['id']]) }}"
              class="flex-1 py-1 rounded-lg text-center text-lg transition-all {{ $currentWeather === $w['id'] ? 'bg-blue-500 border border-blue-400' : 'bg-[#252525] border border-[#333] hover:border-blue-500/50' }}">
              {{ $w['icon'] }}
            </a>
            @endforeach
          </div>
      ...
      <div class="bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4">
        ...
        <div class="flex items-center gap-3 {{ $mealPeriod['peak'] ? 'text-[#FFD23F]' : 'text-gray-300' }}">
          <span class="text-3xl">{{ $mealPeriod['emoji'] }}</span>
          <div>
            <div class="font-black text-lg">{{ $mealPeriod['name'] }}</div>
            @if($mealPeriod['peak'])
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4 text-center">
          <div class="text-[#FFD23F] font-black text-2xl">{{ $pendingRecs }}</div>
          <div class="text-gray-400 text-xs mt-1">Gợi ý chờ</div>
        </div>
        <div class="bg-[#1A1A1A] border-2 border-[#333] rounded-2xl p-4 text-center">
          <div class="text-green-400 font-black text-2xl">{{ $acknowledgedRecs }}</div>
        ...
        <div class="p-4">
          <div class="flex items-start gap-3 mb-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1 flex-wrap">
                <span class="text-white font-black">{{ $rec->inventoryItem?->name ?? 'Nguyên liệu' }}</span>
                <span class="text-[10px] font-black px-2 py-0.5 rounded-full {{ $cfg['badge'] }}">{{ $cfg['label'] }}</span>
              </div>
              <p class="text-gray-300 text-xs">Dự báo từ lịch sử 30 ngày + thời tiết {{ $weather['label'] }}</p>
            </div>
          </div>
          ...
          <div class="bg-black/30 rounded-xl p-3 mb-3">
            ...
            <div class="flex items-center gap-3">
              <span class="text-white font-black text-lg">{{ $current }}</span>
              ...
              <span class="text-[#FFD23F] font-black text-lg">{{ $rec->predicted_qty }}</span>
            </div>
            <div class="flex justify-between text-[10px] text-gray-500 mt-1">
              <span>{{ $rec->inventoryItem?->unit ?? 'đơn vị' }} hiện có</span>
              <span class="{{ $progress < 70 ? 'text-red-400 font-bold' : 'text-green-400' }}">
                {{ $progress < 100 ? 'Thiếu ' . max(0, $rec->predicted_qty - $current) . ' ' . ($rec->inventoryItem?->unit ?? '') : 'Đủ!' }}
              </span>
            </div>
          </div>
