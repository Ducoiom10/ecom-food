...
<head>
  ...
  <style>body { font-family: 'Segoe UI', sans-serif; } .neo-shadow { box-shadow: 4px 4px 0px #1C1C1C; }</style>
</head>
...
<div class="w-full max-w-md">
  <div class="text-center mb-8">
    ...
    <p class="text-gray-500 text-sm mt-1">Nhập email để nhận link đặt lại mật khẩu</p>
  </div>
  <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl neo-shadow p-6">
    @if(session('sent'))
    <div class="bg-green-50 border-2 border-green-300 rounded-xl px-4 py-3 mb-4 text-green-700 text-sm font-bold">
    ...
    <form action="{{ route('auth.forgot.post') }}" method="POST" class="space-y-4">
      ...
      <div>
        ...
        <input type="email" name="email" placeholder="minhtuan@email.com" required
          class="w-full border-2 border-[#1C1C1C] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#FF6B35] transition-all" />
      </div>
      <button type="submit" class="w-full bg-[#FF6B35] text-white font-black py-3.5 rounded-xl border-2 border-[#1C1C1C] neo-shadow hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
        📧 Gửi link đặt lại
      </button>
    </form>
  </div>
