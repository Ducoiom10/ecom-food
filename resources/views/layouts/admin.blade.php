...
<head>
  ...
  <style>
    ...
    .neo-shadow { box-shadow: 4px 4px 0px #333; }
    ...
    @media (min-width: 1024px) and (max-width: 1279px) {
      .nav-item .nav-tooltip {
        display: block; opacity: 0; pointer-events: none; position: absolute;
        left: calc(100% + 10px); top: 50%; transform: translateY(-50%);
        background: #1C1C1C; color: white; font-size: 12px; font-weight: 700;
        padding: 5px 12px; border-radius: 8px; white-space: nowrap;
        transition: opacity .15s ease; z-index: 999; border: 1px solid #444;
      }
      .nav-item .nav-tooltip::before {
        content: ''; position: absolute; right: 100%; top: 50%; transform: translateY(-50%);
        border: 5px solid transparent; border-right-color: #444;
      }
      .nav-item:hover .nav-tooltip { opacity: 1; pointer-events: auto; }
    }
...
<div class="min-h-screen flex">
  ...
  <aside id="sidebar"
    ...
    <div class="p-4 border-b-2 border-[#333] flex items-center gap-3 min-h-[72px]">
      <div class="w-9 h-9 bg-[#FF6B35] border-2 border-[#FF6B35] rounded-xl flex items-center justify-center flex-shrink-0">
      ...
      <button onclick="toggleSidebar()" class="ml-auto lg:hidden text-gray-400 hover:text-white text-lg">✕</button>
    </div>
    ...
    <nav class="flex-1 p-2 space-y-1 overflow-y-auto">
      @php
        $navItems = [
          ['route'=>'admin.kds',       'icon'=>'👨🍳', 'label'=>'Bếp KDS',     'color'=>'bg-orange-500', 'perm'=>'view_kds'],
          ['route'=>'admin.smartprep', 'icon'=>'🧠',   'label'=>'Smart Prep',  'color'=>'bg-purple-500', 'perm'=>'view_smartprep'],
          ['route'=>'admin.dispatch',  'icon'=>'🚚',   'label'=>'Điều phối',   'color'=>'bg-blue-500',   'perm'=>'view_dispatch'],
          ['route'=>'admin.branch',    'icon'=>'🍴',   'label'=>'Chi nhánh',   'color'=>'bg-green-500',  'perm'=>'view_branch'],
          ['route'=>'admin.super',     'icon'=>'📊',   'label'=>'Super Admin', 'color'=>'bg-red-500',    'perm'=>'manage_permissions'],
        ];
      @endphp
  ...
  <div class="flex-1 flex flex-col min-w-0">
    ...
    {{-- Top bar --}}
    <header class="bg-[#1A1A1A] border-b-2 border-[#333] px-4 lg:px-6 py-3 flex items-center justify-between flex-shrink-0 sticky top-0 z-30">
      <div class="flex items-center gap-3">
        <button onclick="toggleSidebar()" class="lg:hidden w-9 h-9 bg-[#252525] border border-[#444] rounded-xl flex items-center justify-center text-gray-400 hover:text-white">
          ☰
        </button>
        <div>
          ...
          <p class="text-gray-400 text-xs hidden sm:block mt-0.5">
            Hệ thống quản trị Ba Anh Em · {{ now()->locale('vi')->isoFormat('dddd, D [tháng] M') }}
          </p>
        </div>
      <div class="flex items-center gap-2 lg:gap-3">
        <div class="hidden sm:flex items-center gap-2 bg-green-500/10 border border-green-500/30 px-3 py-1.5 rounded-lg">
          <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
          <span class="text-green-400 text-xs font-bold">Hoạt động</span>
        </div>
        <button class="relative w-8 h-8 bg-[#252525] border border-[#444] rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-colors">
          🔔
          <span class="absolute -top-1 -right-1 w-4 h-4 bg-[#FF6B35] rounded-full text-[9px] text-white font-black flex items-center justify-center">3</span>
        </button>
        <div class="w-8 h-8 bg-[#FF6B35] rounded-full flex items-center justify-center text-sm font-black cursor-pointer hover:bg-[#e55a25] transition-colors">A</div>
    </header>
