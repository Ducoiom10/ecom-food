    ...
    <div class="mt-4 lg:mt-0 space-y-5">
      ...
      <div class="flex items-center justify-between bg-[#FAFAF8] border-2 border-[#1C1C1C] rounded-xl px-4 py-3 shadow-[2px_2px_0px_#1C1C1C]">
        <div>
          ...
          <div class="font-black text-[#FF6B35] text-2xl lg:text-3xl">{{ number_format($product->base_price) }}đ</div>
        </div>
      ...
      <div>
        <h3 class="font-black text-[#1C1C1C] text-base mb-3">
          {{ $option->name }}
          @if(!$isRequired)<span class="text-gray-400 font-medium text-sm">(tuỳ chọn)</span>@endif
        </h3>
        ...
        <div class="flex gap-2 flex-wrap">
          ...
          <label class="flex-1 cursor-pointer min-w-[80px]">
            <input type="radio" name="option_{{ $option->id }}" value="{{ $val->id }}"
              class="hidden peer" {{ $loop->first ? 'checked' : '' }}
              onchange="updateOptionPrice({{ $val->extra_price }})" />
            <div class="peer-checked:bg-[#FF6B35] peer-checked:text-white peer-checked:border-[#FF6B35] peer-checked:shadow-none
              border-2 border-[#1C1C1C] rounded-xl py-2.5 text-center text-sm font-bold cursor-pointer transition-all shadow-[2px_2px_0px_#1C1C1C] hover:shadow-none">
              <div>{{ $val->label }}</div>
              @if($val->extra_price > 0)
              <div class="text-xs opacity-80">+{{ number_format($val->extra_price) }}đ</div>
              @else
            ...
            <div class="flex items-center gap-3">
              <div class="w-5 h-5 border-2 border-[#1C1C1C] rounded-md flex items-center justify-center group-has-[:checked]:bg-[#FF6B35] group-has-[:checked]:border-[#FF6B35] transition-all flex-shrink-0">
                <input type="checkbox" name="toppings[]" value="{{ $val->id }}"
                  class="opacity-0 absolute" onchange="toggleTopping({{ $val->extra_price }}, this.checked)" />
...
let qty = 1, optionExtra = 0, toppingExtra = 0;
...
function updateOptionPrice(extra) {
  optionExtra = extra;
  recalc();
}
...
function recalc() {
  const total = (basePrice + optionExtra + toppingExtra) * qty;
...
function addToCartPDP() {
  ...
  fetch('/cart/add', {
    ...
    body: JSON.stringify({
      ...
      quantity: qty,
      extra_price: optionExtra + toppingExtra,
