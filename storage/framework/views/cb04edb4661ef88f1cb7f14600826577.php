            ...
            <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-3 mb-4">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <p class="text-xs font-bold text-[#1C1C1C]">
                        Mua thêm <span class="text-[#FF6B35] font-black"><?php echo e(number_format($remaining)); ?>đ</span> để được
                ...
                <div class="relative h-3 bg-gray-100 rounded-full overflow-hidden border border-gray-200">
                    ...
                    <span class="absolute text-base"
                        style="left: <?php echo e($pct); ?>%; top: 50%; transform: translate(-50%,-50%)">🚵</span>
                </div>
            </div>
        <?php else: ?>
                ...
                <div class="space-y-3 max-h-[60vh] overflow-y-auto pr-1 custom-scrollbar" id="cart-items">
                    <?php $__empty_1 = true; $__currentLoopData = $cart ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[4px_4px_0px_#1C1C1C] overflow-hidden hover:shadow-[2px_2px_0px_#1C1C1C] transition-all"
                            id="cart-item-<?php echo e($item['id']); ?>" data-options="<?php echo e(json_encode($item['options'] ?? [])); ?>">
                            <div class="flex p-3 gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="min-w-0">
                                            <h3 class="font-black text-[#1C1C1C] text-sm lg:text-base truncate"><?php echo e($item['name']); ?></h3>
                                            <?php if(!empty($item['option_labels'])): ?>
                                                <div class="text-xs text-[#FF6B35] font-medium mt-0.5 flex flex-wrap gap-1">
                                                    <?php $__currentLoopData = $item['option_labels']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <span class="bg-[#FFF5F0] px-1.5 py-0.5 rounded-md"><?php echo e($label); ?></span>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </div>
                                            <?php endif; ?>
                                            <?php if(!empty($item['note'])): ?>
                                                <div class="text-xs text-orange-500 mt-1 italic">📝 <?php echo e($item['note']); ?></div>
                                            <?php endif; ?>
                                        </div>
                                        <span class="font-black text-[#FF6B35] text-base whitespace-nowrap" id="item-price-<?php echo e($item['id']); ?>"
                                    ...
                                    <div class="flex items-center justify-between mt-3">
                                        <div class="flex items-center gap-1">
                                            <button type="button"
                                                onclick="updateQty('<?php echo e($item['id']); ?>', -1, <?php echo e($item['price']); ?>)"
                                        ...
                                        <div class="flex items-center gap-1">
                                            <button type="button" onclick="openEditModal('<?php echo e($item['id']); ?>')"
            ...
            <div class="mt-6 lg:mt-0 space-y-4 lg:sticky lg:top-24 lg:self-start">
                ...
                <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-4">
                    ...
                    <div id="voucher-list" class="mt-3 space-y-2 hidden">
                        <?php $__empty_1 = true; $__currentLoopData = $vouchers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div onclick="applyVoucher('<?php echo e($v->code); ?>', <?php echo e($v->value); ?>, '<?php echo e($v->type); ?>', <?php echo e($v->max_discount ?? 'null'); ?>)"
                                ...
                                <div class="flex-1">
                                    <div class="font-black text-[#1C1C1C] text-sm"><?php echo e($v->code); ?></div>
                                    <div class="text-xs text-gray-500">
                                        <?php if($v->type === 'flat'): ?>
                                            Giảm <?php echo e(number_format($v->value)); ?>đ
                                        <?php elseif($v->type === 'percent'): ?>
                                            Giảm
                                            <?php echo e($v->value); ?>%<?php echo e($v->max_discount ? ' tối đa ' . number_format($v->max_discount) . 'đ' : ''); ?>

                                        <?php else: ?>
                                            Miễn phí vận chuyển
                                        <?php endif; ?>
                                        <?php if($v->min_order > 0): ?>
                                            · Đơn từ <?php echo e(number_format($v->min_order)); ?>đ
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-4">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <?php $__currentLoopData = [['id' => 'momo', 'label' => 'MoMo', 'emoji' => '💜'], ['id' => 'cod', 'label' => 'Tiền mặt', 'emoji' => '💵'], ['id' => 'zalopay', 'label' => 'ZaloPay', 'emoji' => '🔵'], ['id' => 'bank', 'label' => 'Chuyển khoản', 'emoji' => '🏦']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <button type="button" onclick="selectPayment('<?php echo e($pm['id']); ?>')"
                                id="pm-<?php echo e($pm['id']); ?>"
                                class="flex items-center gap-2 py-2.5 px-3 rounded-xl border-2 text-sm font-bold transition-all <?php echo e($pm['id'] === 'momo' ? 'border-[#FF6B35] bg-orange-50 text-[#FF6B35]' : 'border-gray-200 text-gray-600 hover:border-gray-300'); ?>">
                                <?php echo e($pm['emoji']); ?> <?php echo e($pm['label']); ?>
                ...
                <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between text-gray-600">
                            <span>Tạm tính</span>
                            <span id="subtotal-display"><?php echo e(number_format($subtotal ?? 0)); ?>đ</span>
                        </div>
                        <div class="flex justify-between text-gray-600">
                        ...
                        <div
                            ...
                            <span class="text-[#FF6B35]"
                                id="total-display"><?php echo e(number_format(($subtotal ?? 0) + 15000)); ?>đ</span>
                        </div>
                    </div>
                </div>

                
                <form action="<?php echo e(route('client.checkout.post')); ?>" method="POST" id="checkout-form">
                    <?php echo csrf_field(); ?>
                    ...
                    <button type="submit"
                        ...
                        ⚡ Đặt hàng ngay · <span id="btn-total"><?php echo e(number_format(($subtotal ?? 0) + 15000)); ?>đ</span>
                    </button>
                </form>
...
function saveToppings() {
        ...
        function applyVoucher(code, value, type, maxDiscount) {
            ...
            fetch('<?php echo e(route('client.cart.voucher')); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ecom-food\resources\views/client/cart/index.blade.php ENDPATH**/ ?>