...
<div id="skeleton-menu" class="p-4 lg:p-8 max-w-7xl mx-auto">
  ...
  <div class="flex gap-2 mb-5"><?php for($i=0;$i<5;$i++): ?><div class="skeleton h-9 w-20 flex-shrink-0"></div><?php endfor; ?></div>
...
<div class="p-4 lg:p-8 max-w-7xl mx-auto">
  ...
  <div class="flex gap-3 mb-5">
    <form action="<?php echo e(route('client.menu')); ?>" method="GET" class="flex-1 relative">
      ...
      <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Tìm trong thực đơn..."
  ...
  <div id="filter-panel" class="hidden mb-5 bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[4px_4px_0px_#1C1C1C] p-4">
    ...
    <div class="flex flex-wrap gap-2">
      <?php $__currentLoopData = [['id'=>'popular','label'=>'Phổ biến nhất'],['id'=>'price_asc','label'=>'Giá tăng dần'],['id'=>'price_desc','label'=>'Giá giảm dần']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <a href="<?php echo e(route('client.menu', array_merge(request()->all(), ['sort' => $opt['id']]))); ?>"
        class="text-xs font-bold px-3 py-1.5 rounded-xl border-2 border-[#1C1C1C] transition-all
               <?php echo e(request('sort','popular') === $opt['id'] ? 'bg-[#FF6B35] text-white shadow-[2px_2px_0px_#1C1C1C]' : 'bg-white text-[#1C1C1C] shadow-[2px_2px_0px_#1C1C1C] hover:shadow-none'); ?>">
  ...
  <div class="flex gap-2 overflow-x-auto scrollbar-hide pb-2 mb-5 lg:flex-wrap lg:overflow-visible">
    <a href="<?php echo e(route('client.menu', array_merge(request()->except('category'), ['category' => 'all']))); ?>"
      class="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 border-[#1C1C1C] text-xs lg:text-sm font-bold transition-all
             <?php echo e(request('category','all') === 'all' ? 'bg-[#FF6B35] text-white shadow-[2px_2px_0px_#1C1C1C]' : 'bg-white text-[#1C1C1C] shadow-[2px_2px_0px_#1C1C1C] hover:shadow-none'); ?>">
      🍽️ Tất cả
    </a>
    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <a href="<?php echo e(route('client.menu', array_merge(request()->except('category'), ['category' => $cat->slug]))); ?>"
      class="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 border-[#1C1C1C] text-xs lg:text-sm font-bold transition-all
             <?php echo e(request('category') === $cat->slug ? 'bg-[#FF6B35] text-white shadow-[2px_2px_0px_#1C1C1C]' : 'bg-white text-[#1C1C1C] shadow-[2px_2px_0px_#1C1C1C] hover:shadow-none'); ?>">
  ...
  <p class="text-xs text-gray-500 mb-4"><?php echo e($menuItems->count()); ?> món</p>

  <?php if($menuItems->isEmpty()): ?>
  <div class="text-center py-20">
    <div class="text-6xl mb-4">🍜</div>
    <p class="font-black text-[#1C1C1C] text-xl">Không tìm thấy món nào</p>
    <a href="<?php echo e(route('client.menu')); ?>" class="mt-4 inline-block bg-[#FF6B35] text-white font-black px-5 py-2.5 rounded-xl border-2 border-[#1C1C1C] shadow-[3px_3px_0px_#1C1C1C]">Xem tất cả</a>
  </div>
  <?php endif; ?>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
    ...
    <div class="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[4px_4px_0px_#1C1C1C] overflow-hidden hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all group">
      <a href="<?php echo e(route('client.product', $item->id)); ?>" class="flex sm:flex-col">
        ...
        <div class="flex-1 p-3">
          <div class="font-black text-[#1C1C1C] text-sm leading-tight"><?php echo e($item->name); ?></div>
          <div class="text-gray-500 text-xs mt-1 line-clamp-2"><?php echo e($item->description); ?></div>
          <div class="flex items-center gap-2 mt-2 text-[10px] text-gray-400">
            <span><?php echo e($item->category?->name); ?></span>
            <?php if($item->calories): ?><span>· <?php echo e($item->calories); ?> kcal</span><?php endif; ?>
          </div>
        </div>
      </a>
      <div class="px-3 pb-3 flex items-center justify-between">
        <span class="font-black text-[#FF6B35] text-base"><?php echo e(number_format($item->base_price)); ?>đ</span>
        <div class="flex items-center gap-2">
          <button onclick="removeFromCart(<?php echo e($item->id); ?>)" id="btn-minus-<?php echo e($item->id); ?>"
          ...
          <span id="qty-<?php echo e($item->id); ?>" class="font-black text-[#1C1C1C] text-sm min-w-[20px] text-center hidden">0</span>
...
let cart = {};
const prices = <?php echo json_encode($menuItems->pluck('base_price', 'id'), 512) ?>;
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ecom-food\resources\views/client/menu/index.blade.php ENDPATH**/ ?>