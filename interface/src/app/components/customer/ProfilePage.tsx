...
import { Star, Clock, RotateCcw, ChevronRight, Award, Bell, Shield, HelpCircle, LogOut, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="pb-4">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-gray-500 text-xs">Thành viên từ 01/2026</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-4 -mt-4 relative z-10">
        <div className="bg-[#FFD23F] border-2 border-[#1C1C1C] rounded-2xl shadow-[4px_4px_0px_#1C1C1C] p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-[#1C1C1C]/60 text-xs font-bold uppercase">Snack Points</p>
            <p className="font-black text-[#1C1C1C] text-3xl">{snackPoints.toLocaleString()}</p>
            <p className="text-[#1C1C1C]/70 text-xs mt-0.5">≈ {formatPrice(snackPoints * 100)} · Đủ dùng 1 bữa miễn phí!</p>
          </div>
          <div>
      ...
      <div className="flex px-4 mt-4 gap-2">
        {(["orders", "loyalty"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2.5 rounded-xl border-2 border-[#1C1C1C] text-sm font-black transition-all ${tab === t ? "bg-[#FF6B35] text-white shadow-[2px_2px_0px_#1C1C1C]" : "bg-white text-[#1C1C1C] shadow-[2px_2px_0px_#1C1C1C]"}`}
          >
      ...
      {tab === "orders" && (
        <div className="px-4 mt-4 space-y-3">
          {ORDER_HISTORY.map((order) => (
            <div key={order.id} className="bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <span className="font-black text-[#1C1C1C] text-sm">{order.id}</span>
                  <div className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
                    <Clock size={10} />
                    <span>{order.date}</span>
                  </div>
                </div>
                <span className="bg-green-100 text-green-600 text-xs font-black px-2 py-0.5 rounded-full border border-green-200">✓ Hoàn thành</span>
              </div>
              <div className="text-xs text-gray-500 mb-3">{order.items.join(" · ")}</div>
              <div className="flex items-center justify-between">
                <span className="font-black text-[#FF6B35]">{formatPrice(order.total)}</span>
                <button className="flex items-center gap-1.5 bg-[#FFD23F] text-[#1C1C1C] text-xs font-black px-3 py-2 rounded-xl border-2 border-[#1C1C1C] shadow-[2px_2px_0px_#1C1C1C] hover:shadow-[1px_1px_0px_#1C1C1C] transition-all">
      ...
      {tab === "loyalty" && (
        <div className="px-4 mt-4 space-y-3">
          {[
            ...
            <div key={i} className={`bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] p-4 ${challenge.completed ? "opacity-70" : ""}`}>
              <div className="flex items-center gap-3">
                <span className="text-2xl">{challenge.emoji}</span>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="font-black text-[#1C1C1C] text-sm">{challenge.title}</span>
                    <span className="text-[#FFD23F] font-black text-sm">+{challenge.points} pts</span>
                  </div>
                  <div className="flex items-center gap-2 mt-1.5">
                    <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden border border-gray-200">
                      <div className="h-full bg-[#FF6B35] rounded-full" style={{ width: `${(challenge.progress / challenge.max) * 100}%` }} />
                    </div>
                    <span className="text-xs text-gray-500">{challenge.progress}/{challenge.max}</span>
                  </div>
                </div>
              </div>
              {challenge.completed && (
                <div className="mt-2 bg-green-50 border border-green-200 rounded-xl px-3 py-1.5 text-xs text-green-600 font-bold text-center">✓ Đã hoàn thành!</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="px-4 mt-6 space-y-2">
        {[
          { icon: Bell, label: "Thông báo", sub: "Email & Push notification" },
          { icon: Shield, label: "Bảo mật", sub: "Đổi mật khẩu, 2FA" },
          { icon: HelpCircle, label: "Trợ giúp", sub: "FAQ, Báo cáo sự cố" },
        ].map(({ icon: Icon, label, sub }) => (
          <button key={label} className="w-full bg-white border-2 border-[#1C1C1C] rounded-2xl shadow-[3px_3px_0px_#1C1C1C] px-4 py-3 flex items-center gap-3 hover:shadow-[1px_1px_0px_#1C1C1C] hover:translate-x-[1px] hover:translate-y-[1px] transition-all">
